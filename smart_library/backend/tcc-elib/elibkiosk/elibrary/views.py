...
def programs(request):
    top_accessed_book = None
    recommended_book = None

    top_accessed_data = (
        LibraryAction.objects
        .filter(content_type="book", action="recorded")
        .values("content_id")
    ...
    if top_accessed_data:
        top_accessed_book = Book.objects.filter(id=top_accessed_data["content_id"]).first()

    if top_accessed_book:
        recommended_book = (
            Book.objects
            .exclude(id=top_accessed_book.id)
            .order_by("title")
            .first()
        )
    else:
        recommended_book = Book.objects.order_by("title").first()

    return render(
        request,
        "catalog/programs.html",
        {
            "top_accessed_book": top_accessed_book,
...
def program_books(request, program_code):
    ...
    if book_ids:
        ...
        if top_accessed_data:
            top_accessed_book = Book.objects.filter(
                id=top_accessed_data["content_id"]
            ).first()

    # fallback / secondary card
    if top_accessed_book:
        recommended_book = books.exclude(id=top_accessed_book.id).first()
    else:
        top_accessed_book = books.first()
        recommended_book = books.exclude(id=top_accessed_book.id).first() if top_accessed_book else None
...
def thesis_list(request, program_code):
    if program_code == "best":
        theses = Thesis.objects.filter(is_best=True).order_by("-year")
        program = None
        page_title = "Best Theses"
    else:
        program = get_object_or_404(Program, code=program_code)
...
def consume_scan(request):
    ...
    try:
        ...
        request.session["rfid_user_id_number"] = log.user.id_number
...
def record_book(request):
    ...
    # 🔥 STORE INTO SESSION
    request.session["rfid_user_id"] = user_id
